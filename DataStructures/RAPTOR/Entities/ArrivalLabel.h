...
struct WalkingParetoLabel {
    ...
    inline bool isWithinSlack(const std::vector<WalkingParetoLabel>& anchorLabels, const double arrivalFactor, const double tripsPerHour) const noexcept {
        for (const WalkingParetoLabel& anchorLabel : anchorLabels) {
            if (anchorLabel.arrivalTime > arrivalTime) continue;
            if (anchorLabel.numberOfTrips > numberOfTrips) continue;
            if (!isWithinSlack(anchorLabel, arrivalFactor, tripsPerHour)) return false;
        }
        return true;
    }

    inline bool isWithinSlack(const WalkingParetoLabel& anchorLabel, const double arrivalFactor, const double tripsPerHour) const noexcept {
        if (arrivalTime > anchorLabel.arrivalTime + anchorLabel.walkingDistance * arrivalFactor) return false;
        if (numberOfTrips > anchorLabel.numberOfTrips + anchorLabel.walkingDistance/static_cast<double>(3600) * tripsPerHour) return false;
        return true;
    }
...
inline constexpr int TripFactor = 60 * 60;
inline Geometry::Halfplane getSeparatingHalfplane(const WalkingParetoLabel& a, const WalkingParetoLabel& b) noexcept {
    const int arrivalDiff = a.arrivalTime - b.arrivalTime;
    const int walkingDiff = a.walkingDistance - b.walkingDistance;
    const int tripDiff = (a.numberOfTrips - b.numberOfTrips) * TripFactor;
    const double divisor = static_cast<double>(walkingDiff - tripDiff);
    const double x = -(arrivalDiff - tripDiff)/divisor;
    const double y = -tripDiff/divisor;
    return Geometry::Halfplane(x, y, divisor < 0);
}

inline std::vector<RAPTOR::WalkingParetoLabel> getPreferenceOptimalLabels(const std::vector<RAPTOR::WalkingParetoLabel>& labels, const double arrivalFactor, const double tripsPerHour) noexcept {
    std::vector<RAPTOR::WalkingParetoLabel> result;
    for (size_t i = 0; i < labels.size(); i++) {
        std::vector<Geometry::Halfplane> halfplanes;
        for (size_t j = 0; j < labels.size(); j++) {
            if (i == j) continue;
            const Geometry::Halfplane line = getSeparatingHalfplane(labels[i], labels[j]);
            halfplanes.emplace_back(line);
        }
        if (intersectHalfplanes(halfplanes, arrivalFactor, tripsPerHour).empty()) continue;
        result.emplace_back(labels[i]);
    }
    return result;
}
