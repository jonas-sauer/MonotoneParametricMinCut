...
private:
    ...
    inline void computeShortcuts() {
        ...
        RAPTOR::ULTRA::Builder shortcutGraphBuilder(data);
    ...
    inline void computeCoreCH() {
        ...
        using PROFILER = CH::TimeProfiler;
        using WITNESS_SEARCH = CH::BidirectionalWitnessSearch<CHCoreGraph, PROFILER, 200>;
        ...
        GREEDY_KEY_FUNCTION greedyKey(1024, 256, 0);
        CH::Builder<PROFILER, WITNESS_SEARCH, KEY_FUNCTION, STOP_CRITERION, false, false> chBuilder(std::move(inputGraph), inputGraph[TravelTime], KEY_FUNCTION(isNormalVertex, inputGraph.numVertices(), greedyKey), STOP_CRITERION(numberOfCoreVertices, 16));
        chBuilder.run();
    ...
    inline void computeCH() {
        ...
        KEY_FUNCTION greedyKey(1024, 512, 0);
        CH::Builder<PROFILER, WITNESS_SEARCH, KEY_FUNCTION, STOP_CRITERION, false, false> chBuilder(std::move(inputGraph), inputGraph[TravelTime], greedyKey);
        chBuilder.run();
        chBuilder.copyCoreToCH();
