...
#include "../../Algorithms/RAPTOR/BikeSharing/OperatorHullTimestampRangeRAPTOR.h"
...
public:
    static inline std::vector<bool> generateBicycleTransportFlags(const Intermediate::Data& data, const double seed = 42.0) noexcept {
        ...
        for (size_t i = 0; i < result.size(); i++) {
            switch (data.trips[i].type) {
                ...
                case GTFS::Type::Ferry: {
                    result[i] = true;
                    break;
                }
                case GTFS::Type::CableCar: {
                    result[i] = false;
                    break;
                }
                case GTFS::Type::Gondola: {
                    result[i] = false;
                    break;
                }
                case GTFS::Type::Funicular: {
                    result[i] = false;
                    break;
                }
                default: {
    ...
    inline void computeCoreCH(RAPTOR::Data& data, CH::CH& coreCH) noexcept {
        ...
        CH::Builder<PROFILER, WITNESS_SEARCH, KEY_FUNCTION, STOP_CRITERION, false, false> chBuilder(std::move(graph), KEY_FUNCTION(isNormalVertex, graph.numVertices(), greedyKey), STOP_CRITERION(numberOfCoreVertices, 14));
        chBuilder.run();
