...
public:
    AssignmentWorker(const CSA::Data& data, const CSA::TransferGraph& reverseGraph, const std::vector<double>& connectionCapacity, std::vector<double>& connectionLoad, const Settings& settings, const DecisionModel& decisionModel) :
        data(data),
        reverseGraph(reverseGraph),
        connectionCapacity(connectionCapacity),
        settings(settings),
        decisionModel(decisionModel),
        patComputation(data, reverseGraph, settings, connectionCapacity, connectionLoad, patData),
        patData(1, PATData(data.numberOfStops(), data.numberOfConnections())),
    ...
    inline void run(const Vertex destinationVertex, std::vector<AccumulatedVertexDemand::Entry>& demand) noexcept {
        ...
        patComputation.run(destinationVertex, 0);
...
private:
    inline void initializeAssignment(const std::vector<AccumulatedVertexDemand::Entry>& demand) noexcept {
        ...
        for (const StopId stop : data.stops()) {
            patData[0].profiles.resetScanIndex(stop);
    ...
    inline void evaluateInitialStop(const AccumulatedVertexDemand::Entry& demandEntry, const Vertex stop, const int transferTime, ChoiceSet& choiceSet) noexcept {
        ...
        while (departureTime <= latestDepartureTime) {
            const ProfileEntry& entry = patData[0].profiles.findEntry(stop, departureTime);
    ...
    inline void processConnection(const ConnectionId i) noexcept {
        ...
        const ConnectionLabel& label = patData[0].connectionLabels[i];
        ...
        const double targetPAT = patData[0].targetPAT(connection);
        const double hopOffPAT = std::min(targetPAT, label.transferPAT) + hopOffLoadCost;
        const double hopOnPAT = std::min(hopOffPAT, label.tripPAT) + hopOnLoadCost + travelLoadCost;
