...
#include "NewBackwardPruningULTRARAPTOR.h"
...
public:
    ...
    using Type = NewUBMRAPTOR<Profiler>;
...
public:
    NewUBMRAPTOR(const Data& data, const Data& backwardData, const CH::CH& chData, const Profiler& profilerTemplate = Profiler()) :
        data(data),
    ...
    inline void run(const Vertex source, const int departureTime, const Vertex target, const double arrivalTimeFactor, const double tripsPerHour) noexcept {
        profiler.start();
        ...
        forwardPruningRAPTOR.run(source, departureTime, target, arrivalTimeFactor, tripsPerHour);
        profiler.doneRound();
        if (forwardPruningRAPTOR.getAnchorLabels().empty()) return;
        profiler.startExtraRound(EXTRA_ROUND_BACKWARD_PRUNING);
        backwardPruningRAPTOR.run(target, source, departureTime, arrivalTimeFactor, tripsPerHour);
        profiler.doneRound();
        ...
        maxTrips = backwardPruningRAPTOR.getMaxTrips();
        initialize();
    ...
    inline void verify(const double arrivalTimeFactor, const double tripsPerHour) const noexcept {
        const std::vector<WalkingParetoLabel>& anchorLabels = forwardPruningRAPTOR.getAnchorLabels();
        for (const WalkingParetoLabel& anchorLabel : anchorLabels) {
            Ensure(isContained(anchorLabel), "Anchor label " << anchorLabel << " was not found!");
        }
        for (const WalkingParetoLabel& label : getResults()) {
            if (!label.isWithinSlack(anchorLabels, arrivalTimeFactor, tripsPerHour)) {
                std::cout << "Label " << label << " exceeds slack!" << std::endl;
...
private:
    ...
    inline bool isContained(const WalkingParetoLabel& anchorLabel) const noexcept {
        ...
        for (const Label& label : rounds[round][targetStop]) {
            if (label.arrivalTime == anchorLabel.arrivalTime && label.walkingDistance <= anchorLabel.walkingDistance) return true;
        }
        return false;
    }
...
private:
    ...
    Profiler profiler;
    NewForwardPruningULTRARAPTOR<Profiler> forwardPruningRAPTOR;
    NewBackwardPruningULTRARAPTOR<Profiler> backwardPruningRAPTOR;
