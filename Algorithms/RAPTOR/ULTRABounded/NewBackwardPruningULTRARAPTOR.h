...
#include "NewForwardPruningULTRARAPTOR.h"

namespace RAPTOR {

template<typename PROFILER, typename INITIAL_TRANSFERS = BucketCHInitialTransfers>
class NewBackwardPruningULTRARAPTOR {

public:
    using Profiler = PROFILER;
    using InitialTransferType = INITIAL_TRANSFERS;
    using Type = NewBackwardPruningULTRARAPTOR<Profiler, InitialTransferType>;
...
public:
    NewBackwardPruningULTRARAPTOR(const Data& data, const InitialTransferType& initialTransfers, const NewForwardPruningULTRARAPTOR<Profiler, InitialTransferType>& forwardPruningRAPTOR, Profiler& profiler) :
        data(data),
    ...
    inline void run(const Vertex source, const Vertex target, const int, const double arrivalFactor, const double tripsPerHour) noexcept {
        ...
        Bag<ArrivalLabel> pruningBag;
        for (const WalkingParetoLabel& label : forwardPruningRAPTOR.getAnchorLabels()) {
            const int slackArrivalTime = label.arrivalTime + label.walkingDistance * arrivalFactor;
            const size_t slackNumTrips = label.numberOfTrips + label.walkingDistance/static_cast<double>(3600) * tripsPerHour;
            pruningBag.merge(ArrivalLabel(label.arrivalTime, slackNumTrips + 1));
            pruningBag.merge(ArrivalLabel(slackArrivalTime + 1, label.numberOfTrips));
        }
        std::vector<ArrivalLabel> pruningLabels = pruningBag.labels;
        std::sort(pruningLabels.begin(), pruningLabels.end());
        std::vector<ArrivalLabel> searchLabels;
        maxTrips = 0;
        for (size_t i = 0; i + 1 < pruningLabels.size(); i++) {
            maxTrips = std::max(maxTrips, pruningLabels[i].numberOfTrips - 1);
            searchLabels.emplace_back(pruningLabels[i+1].arrivalTime - 1, pruningLabels[i].numberOfTrips - 1);
        }

        for (const ArrivalLabel& label : searchLabels) {
            sourceDepartureTime = -label.arrivalTime;
            roundOffset = 2 * (maxTrips - label.numberOfTrips);
            runIteration();
        }
...
private:
    ...
    inline void relaxInitialTransfers() noexcept {
        for (const Vertex stop : initialTransfers.getBackwardPOIs()) {
            ...
            Assert(initialTransfers.getBackwardDistance(stop) != INFTY, "Vertex " << stop << " was not reached!");
...
private:
    ...
    const NewForwardPruningULTRARAPTOR<Profiler, InitialTransferType>& forwardPruningRAPTOR;
