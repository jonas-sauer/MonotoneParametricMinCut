...
public:
    ...
    using Type = UBMTBRAPTOR<Profiler>;
...
public:
    UBMTBRAPTOR(const Data& data, const TripBased::Data& forwardTBData, const TripBased::Data& backwardTBData, const CH::CH& chData, const Profiler& profilerTemplate = Profiler()) :
        data(data),
...
private:
    ...
    inline void targetArrivalByTransfer(const Label& label) noexcept {
        ...
        Assert(bestLabels[targetStop].dominates(currentRound()[targetStop]), "Best bag does not dominate current bag!");
    }
