...
template<bool DEBUG>
class OperatorHullTimestampRangeRAPTOR {

public:
    inline constexpr static bool Debug = DEBUG;
    using Type = OperatorHullTimestampRangeRAPTOR<Debug>;

private:
    struct RoundLabel : public ExternalKHeapElement {
        RoundLabel() : ExternalKHeapElement(), arrivalTime(INFTY), parentIndex(0), routeId(noRouteId), parentVertex(noVertex), timestamp(0) {}
        inline bool hasSmallerKey(const RoundLabel* other) const {
            return arrivalTime < other->arrivalTime;
        }
        inline void update(const int at, const u_int32_t pi, const RouteId r, const Vertex pv, /*const bool cuv,*/ ExternalKHeap<2, RoundLabel>& q) noexcept {
            arrivalTime = at;
            parentIndex = pi;
            routeId = r;
            parentVertex = pv;
...
public:
    OperatorHullTimestampRangeRAPTOR(const RAPTOR::Data& cyclingNetwork) :
...
private:
    ...
    inline bool improveEarliestArrival(const Vertex vertex, const int time) noexcept {
        if (rounds[roundIndex][vertex].timestamp != timestamp) {
            size_t r = roundIndex - 1;
            while (r > 0) {
                rounds[roundIndex][vertex].arrivalTime = std::min(rounds[roundIndex][vertex].arrivalTime, rounds[r][vertex].arrivalTime);
                if (rounds[r][vertex].timestamp == timestamp) break;
                r--;
            }
            if (r == 0) {
                rounds[roundIndex][vertex].arrivalTime = std::min(rounds[roundIndex][vertex].arrivalTime, currentDepartureTime + std::min(rounds[0][vertex].arrivalTime, maximumTravelTime + 1));
            }
            rounds[roundIndex][vertex].timestamp = timestamp;
        }
        return rounds[roundIndex][vertex].arrivalTime > time;
    }
