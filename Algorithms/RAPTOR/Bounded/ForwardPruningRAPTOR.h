...
private:
    ...
    inline void scanRoutes() noexcept {
        ...
        for (const RouteId route : routesServingUpdatedStops.getKeys()) {
            ...
            Assert(trip[stopIndex].departureTime >= previousRound()[stop].arrivalTimeByTransfer, "Cannot scan a route after the last trip has departed (Route: " << route << ", Stop: " << stop << ", StopIndex: " << stopIndex << ", Time: " << previousRound()[stop].arrivalTimeByTransfer << ", LastDeparture: " << trip[stopIndex].departureTime << ")!");
