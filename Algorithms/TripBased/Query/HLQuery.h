...
public:
    HLQuery(const Data& data, const TransferGraph& outHubs, const TransferGraph& inHubs) :
        data(data),
        bucketHL(outHubs, inHubs, data.numberOfStops()),
...
private:
    ...
    inline void computeInitialAndFinalTransfers() noexcept {
        profiler.startPhase();
        bucketHL.run(sourceVertex, targetVertex);
        if (bucketHL.getDistance() != INFTY) {
            addTargetLabel(sourceDepartureTime + bucketHL.getDistance());
        }
    ...
    inline void evaluateInitialTransfers() noexcept {
        ...
        for (const Vertex stop : bucketHL.getForwardPOIs()) {
        ...
        for (const RouteId route : data.routes()) {
            ...
            for (StopIndex stopIndex(0); stopIndex < endIndex; stopIndex++) {
                ...
                const int timeFromSource = bucketHL.getForwardDistance(stop);
        ...
        while (roundBegin < roundEnd) {
            ...
            for (size_t i = roundBegin; i < roundEnd; i++) {
                ...
                for (StopEventId j = label.begin; j < label.end; j++) {
                    ...
                    const int timeToTarget = bucketHL.getBackwardDistance(data.arrivalEvents[j].stop);
    ...
    inline RAPTOR::Journey getJourney(const TargetLabel& targetLabel) const noexcept {
        ...
        const int timeFromSource = bucketHL.getForwardDistance(departureStop);
    ...
    inline std::pair<StopEventId, Edge> getParent(const TripLabel& parentLabel, const TargetLabel& targetLabel) const noexcept {
        ...
        for (StopEventId i = parentLabel.begin; i < end; i++) {
            const int timeToTarget = bucketHL.getBackwardDistance(data.arrivalEvents[i].stop);
