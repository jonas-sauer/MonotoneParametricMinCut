...
class ExcessesIBFS {
    ...
    struct ExcessBuckets {
        ...
        inline void clear() noexcept {
            std::vector<std::vector<Vertex>>(buckets.size()).swap(buckets);
            Vector::fill(positionOfVertex, -1);
            maxBucket = -1;
        }
...
public:
    explicit ExcessesIBFS(const StaticFlowGraph& graph) :
        graph(graph),
        n(graph.numVertices()),
...
public:
    ...
    inline int getFlowValue() const noexcept {
        ...
        for (const auto [edge, from] : graph.edgesWithFromVertex()) {
            ...
            if (distance[from] >= 0 && distance[to] < 0) flow += graph.get(Capacity, edge) - residualCapacity[edge];
        }
        return flow;
    }
...
private:
    ...
    inline void clearDirectional() noexcept {
        ...
        excessVertices[DIRECTION].clear();
    ...
    inline int getInflow(const Vertex vertex) const noexcept {
        ...
        for (const Edge edge : graph.edgesFrom(vertex)) {
            ...
            inflow += graph.get(Capacity, reverseEdge) - residualCapacity[reverseEdge];
        }
        return inflow;
    }
