...
class WritePartitionGraph : public ParameterizedCommand {

public:
    WritePartitionGraph(BasicShell& shell) :
        ParameterizedCommand(shell, "writePartitionGraph", "Generates a HMetis hypergraph for partitioning.") {
        addParameter("RAPTOR binary");
        addParameter("HMetis output file");
        addParameter("Transitive routes?", "false");
    }
    ...
    virtual void execute() noexcept {
        ...
        RAPTOR::TransferGraph transferGraph = getParameter<bool>("Transitive routes?") ? data.minTravelTimeTransitiveGraph() : data.minTravelTimeGraph();
        Graph::printInfo(transferGraph);
        DynamicFlowGraph flowGraph = Graph::generateFlowGraph(transferGraph, true);
        Graph::printInfo(flowGraph);
        flowGraph.deleteEdges([&](const Edge edge){
            return flowGraph.get(ToVertex, edge) <= flowGraph.get(FromVertex, edge);
        });
        flowGraph.packEdges();
        Graph::printInfo(flowGraph);
        flowGraph.printAnalysis();
        std::ofstream hgrOs(FileSystem::ensureExtension(getParameter("HMetis output file"), ".hgr"));
        Assert(hgrOs);
        Assert(hgrOs.is_open());
        hgrOs << flowGraph.numVertices() << " " << flowGraph.numEdges() << std::endl;
        for (const Vertex vertex : flowGraph.vertices()) {
            ...
            if (!edges.empty()) {
                ...
                warning("Vertex ", vertex, " is isolated!");
            }
            hgrOs << "\n";
        }
        hgrOs.close();
...
class ReadHMetisPartition : public ParameterizedCommand {

public:
    ReadHMetisPartition(BasicShell& shell) :
        ParameterizedCommand(shell, "readHMetisPartition", "Loads a HMetis partition.") {
        addParameter("RAPTOR binary");
        addParameter("HMetis input file");
        addParameter("Graphics output file");
        addParameter("Format", { "pdf", "png", "svg" });
        addParameter("Transitive routes?", "false");
    }
...
private:
    ...
    inline void draw() const noexcept {
        ...
        std::cout << "Initializing Inertial Flow." << std::endl;
        std::cout << "Computing Partition." << std::endl;
        NestedDissection nd(transferGraph.numVertices());
        const std::string hMetisFile = getParameter("HMetis input file");
        nd.readHMetis(transferGraph, hMetisFile);
        std::cout << "Drawing Partition." << std::endl;
        Visualization doc = Visualization::FromRAPTOR(getParameter("Graphics output file"), data, 0.3);
        doc.drawPartition(nd, true);
        doc.close();
        std::cout << "Saving Partition." << std::endl;
        nd.serialize(hMetisFile + ".nd");
    }
};
