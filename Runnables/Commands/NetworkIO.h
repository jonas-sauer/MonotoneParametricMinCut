...
class CSAToCSV : public ParameterizedCommand {

public:
    CSAToCSV(BasicShell& shell) :
        ParameterizedCommand(shell, "csaToCSV", "Exports a binary CSA network in a .csv file format.") {
        addParameter("Input file");
        addParameter("Output directory");
        addParameter("Sort connections?", "true");
    }

    virtual void execute() noexcept {
        const std::string inputFile = getParameter("Input file");
        const std::string outputDirectory = getParameter("Output directory");
        const bool sortConnections = getParameter<bool>("Sort connections?");

        CSA::Data csa = CSA::Data::FromBinary(inputFile);
        csa.printInfo();
        if (sortConnections) csa.sortUnique();
        csa.writeCSV(outputDirectory);
    }

};

class CSAToExchangeFormat : public ParameterizedCommand {

public:
    CSAToExchangeFormat(BasicShell& shell) :
        ParameterizedCommand(shell, "csaToExchangeFormat", "Converts binary CSA data to exchange format.") {
        addParameter("Input file");
        addParameter("Output file");
    }
    ...
    virtual void execute() noexcept {
        ...
        CSA::Data csa = CSA::Data::FromBinary(inputFile);
        csa.printInfo();
        csa.writeExchangeFormat(outputFile);
    }
...
public:
    CSAToIntermediate(BasicShell& shell) :
        ParameterizedCommand(shell, "csaToIntermediate", "Converts binary CSA data to intermediate network format.") {
        addParameter("Input file");
        addParameter("Output file");
    }
    ...
    virtual void execute() noexcept {
        ...
        CSA::Data csa = CSA::Data::FromBinary(inputFile);
        csa.printInfo();
        Intermediate::Data data = Intermediate::Data::FromCSA(csa);
        data.printInfo();
        data.serialize(outputFile);
    }
...
public:
    RAPTORToCSV(BasicShell& shell) :
        ParameterizedCommand(shell, "raptorToCSV", "Exports a binary RAPTOR network in a .csv file format.") {
        addParameter("Input file");
        addParameter("Output path");
    }

    virtual void execute() noexcept {
        const std::string inputFile = getParameter("Input file");
        const std::string outputPath = getParameter("Output path");

        RAPTOR::Data raptor = RAPTOR::Data::FromBinary(inputFile);
        raptor.printInfo();
        raptor.writeCSV(outputPath);
    }
...
public:
    RAPTORToIntermediate(BasicShell& shell) :
        ParameterizedCommand(shell, "raptorToIntermediate", "Converts binary RAPTOR data to intermediate network format.") {
        addParameter("Input file");
        addParameter("Output file");
    }
    ...
    virtual void execute() noexcept {
        ...
        inter.printInfo();
        Graph::printInfo(inter.transferGraph);
        inter.transferGraph.printAnalysis();
        inter.serialize(outputFile);
    }
