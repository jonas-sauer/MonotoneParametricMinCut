...
class RunNewUBMRAPTORQueries : public ParameterizedCommand {

public:
    RunNewUBMRAPTORQueries(BasicShell& shell) :
        ParameterizedCommand(shell, "runNewUBMRAPTORQueries", "Runs the given number of random new UBM-RAPTOR queries.") {
        addParameter("RAPTOR input file");
        addParameter("CH data");
        addParameter("Number of queries");
    ...
    virtual void execute() noexcept {
        ...
        const RAPTOR::Data reverseData = raptorData.reverseNetwork();
        CH::CH ch(getParameter("CH data"));
        RAPTOR::NewUBMRAPTOR<RAPTOR::AggregateProfiler> algorithm(raptorData, reverseData, ch);
        ...
        double numJourneys = 0;
        double numPreferenceOptimalJourneys = 0;
        Timer timer;
        double filterTime = 0;
        for (const VertexQuery& query : queries) {
            algorithm.run(query.source, query.departureTime, query.target, arrivalFactor, tripsPerHour);
            const std::vector<RAPTOR::WalkingParetoLabel> labels = algorithm.getResults();
            numJourneys += labels.size();
            timer.restart();
            const std::vector<RAPTOR::WalkingParetoLabel> hullLabels = getPreferenceOptimalLabels(labels, arrivalFactor, tripsPerHour);
            filterTime += timer.elapsedMicroseconds();
            numPreferenceOptimalJourneys += hullLabels.size();
        }

        algorithm.getProfiler().printStatistics();
        std::cout << "Avg. journeys: " << String::prettyDouble(numJourneys/n) << std::endl;
        std::cout << "Avg. preference-optimal journeys: " << String::prettyDouble(numPreferenceOptimalJourneys/n) << std::endl;
        std::cout << "Filtering time: " << String::musToString(filterTime/n) << std::endl;
    }
};

class RunUBMTBRAPTORQueries : public ParameterizedCommand {

public:
    RunUBMTBRAPTORQueries(BasicShell& shell) :
        ParameterizedCommand(shell, "runUBMTBRAPTORQueries", "Runs the given number of random UBM-TB-RAPTOR queries.") {
        addParameter("RAPTOR input file");
    ...
    virtual void execute() noexcept {
        ...
        RAPTOR::UBMTBRAPTOR<RAPTOR::AggregateProfiler> algorithm(raptorData, forwardBoundedData, backwardBoundedData, ch);
