...
public:
    BuildBikeSharingData(BasicShell& shell) :
        ...
        addParameter("Draw reachability hulls", "false", {"true", "false"});
    ...
    virtual void execute() noexcept {
        ...
        const std::string drawReachabilityHulls = getParameter("Draw reachability hulls");
        ...
        if (drawReachabilityHulls == "false") return;
        TimeTableVisualization<PDF> testPDF = TimeTableVisualization<PDF>::FromRAPTOR(outputFile + ".hulls.pdf", data.cyclingNetwork, 0.3);
        for (size_t bikeOperator = 1; bikeOperator < data.numberOfOperators(); bikeOperator++) {
            testPDF.drawBorders(country);
            std::vector<Vertex> rr;
            for (const Vertex v : data.cyclingNetwork.transferGraph.vertices()) {
                if (data.reachableVerticesOfOperator[bikeOperator][v]) {
                    rr.emplace_back(v);
                }
            }
            testPDF.drawVertices(rr, Color::KITblue);
            testPDF.drawVertices(data.verticesOfOperator[bikeOperator], Color::KITred);
            testPDF.newPage();
        }
    }
...
public:
    ...
    virtual void execute() noexcept {
        ...
        BikeSharing::Data data(bikeSharingFile);
        Dijkstra<TransferGraph> dijkstra(data.walkingGraph);
